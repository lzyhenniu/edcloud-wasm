...
const manualPipe = async (readable, writable, close) => {
    ...
    let directChunks = new Array(1024), directCount = 0, directBytes = 0, directTimer = null, isClose = false, fastFlush = true;
    const flushDirect = () => {
        ...
        directCount === 1 && (directChunks[0] = null), directCount = 0, directBytes = 0;
    };
    const flushBuffer = () => {
        ...
        while (true) {
            if (fastFlush) {
                ...
                if (chunkLen >= halfChunkLen) {
                    ...
                    directChunks[directCount++] = value.slice(), directBytes += chunkLen, spareBuffer = value.buffer, flushDirect();
                } else {
